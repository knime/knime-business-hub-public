...
The following Ingress rules can be created before installing Hub via KOTS  along with configuring DNS and TLS. 

Ingress rules may need to be updated depending on use of other tools to correctly set DNS, TLS, etc. 

Additionally, the Ingress rules will need to be updated with the appropriate Ingress Class name. (`nginx` is used below as a placeholder)

```yaml
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: knime-execution-webapp
  namespace: hub
  annotations:
